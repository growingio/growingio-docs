...
在产品和业务中，一般对于“活跃用户”的定义，最基本的定义是访问或者登陆过产品的用户；进一步也可以将做过产品核心价值功能的用户定义为“活跃用户”；例如以下类型：
...
根据 **用户类型、用户事件、频次、事件的属性** 来定义活跃用户的条件
...
选择查看活跃用户的趋势的时间；选择查看活跃用户的时间颗粒度。对于部分高频产品来说，关注颗粒度会在日级别；对于工具型，一般在周级别；对于低频型，颗粒度在月级别。
...
如果只需要一个指标，来了解目前的增长状态，就用增长指数。它展示了，在每个周期（天、周、月）获取的增长用户与这个周期流失的用户的比，**比率表示，每当流失一个活跃用户的时候 （即一个活跃用户不再满足活跃用户条件了），会增加几个活跃用户**。

**增长指数 =（当期新增用户数+当期回流用户数）/ 当期活跃但是在下周期不活跃的用户数**

当增长指数 &gt;1 时，表明即使存在用户流失，你的活跃用户也是在增长的。 当增长指数 &lt;1 时，表明你的活跃用户其实在流失的。 你需要关注增长指数的变化趋势，并且在 &lt;1 时重点关注。

注：当时间颗粒度是周和月时，会存在最后一个周期没有结束的情况（例如今天是周三，但是一周结束需要到周天，所以上周的活跃用户，只有在本周周天结束的时候，才能知道有多少用户本周不再活跃了），此时数据点和线会显示为虚线。

![](../.gitbook/assets/image%20%2820%29.png)

**净增用户 = \(当期新增用户数 + 当期回流用户数\) - 当去活跃但在下周起不活跃用户数**
