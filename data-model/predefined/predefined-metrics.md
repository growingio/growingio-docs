...
* [第一部分:访问级指标](predefined-metrics.md#第一部分：访问级指标)
  * [1.访问量\(web/app\)](predefined-metrics.md#1fang-wen-liang)
  * [2.访问用户人均访问次数（web/app） \[ 访问用户 / 访问量 \]](predefined-metrics.md#2访问用户人均访问次数（webapp）--访问用户--访问量-)
  * [3.每次访问页面浏览量（web/app） \[ 访问量 / 页面浏览量 \]](predefined-metrics.md#3每次访问页面浏览量（webapp）--访问量--页面浏览量-)
  * [4.总访问时长（web/app）](predefined-metrics.md#4总访问时长（webapp）)
  * [5.平均访问时长（web/app） \[ 总访问时长 / 访问量 \]](predefined-metrics.md#5平均访问时长（webapp）--总访问时长--访问量-)
  * [6.退出次数（web/app）](predefined-metrics.md#6退出次数（webapp）)
  * [7.退出率（web/app） \[ 退出次数 / 访问量 \]](predefined-metrics.md#7退出率（webapp）--退出次数--访问量-)
  * [8.页面停留时长（web）](predefined-metrics.md#8页面停留时长（web）)
  * [9.平均页面停留时长（web）\[ 总页面停留时长/（页面浏览量 - 退出次数）\]](predefined-metrics.md#9平均页面停留时长（web）-总页面停留时长（页面浏览量---退出次数）)
  * [10.进入量（web）](predefined-metrics.md#10进入量（web）)
  * [11.访问用户人均进入次数（web） \[ 进入量 / 访问用户量 \]](predefined-metrics.md#11访问用户人均进入次数（web）---进入量--访问用户量-)
  * [12.总进入时长（web）](predefined-metrics.md#12总进入时长（web）)
  * [13.平均进入时长（web） \[ 总进入时长 / 进入量 \]](predefined-metrics.md#13平均进入时长（web）--总进入时长--进入量-)
  * [14.每次进入页面浏览量（web） \[ 进入总页面浏览量 / 进入量 \]](predefined-metrics.md#14每次进入页面浏览量（web）--进入总页面浏览量--进入量-)
  * [15.跳出次数（web）](predefined-metrics.md#15跳出次数（web）)
  * [16.跳出率（web） \[ 跳出次数 / 进入量 \]](predefined-metrics.md#16跳出率（web）--跳出次数--进入量-)
* [第二部分：用户级指标](predefined-metrics.md#第二部分：用户级指标)
  * [17.访问用户量（web/app）](predefined-metrics.md#17访问用户量（webapp）)
  * [18.新访问用户量（web/app）](predefined-metrics.md#18新访问用户量（webapp）)
  * [19.登录用户量（web/app）](predefined-metrics.md#19登录用户量（webapp）)
  * [20.新登录用户量（web/app）](predefined-metrics.md#20新登录用户量（webapp）)
* [第三部分：页面级指标](predefined-metrics.md#第三部分：页面级指标)
  * [21.页面浏览量（web/app）](predefined-metrics.md#21页面浏览量（webapp）)
  * [22.通过圈选定义的页面浏览量（web/app）](predefined-metrics.md#22通过圈选定义的页面浏览量（webapp）)
* [第四部分：事件级指标](predefined-metrics.md#第四部分：事件级指标)
  * [23.通过圈选定义的元素指标化后（web/app）](predefined-metrics.md#23通过圈选定义的元素指标化后（webapp）)
  * [24.自定义事件（web/app）](predefined-metrics.md#24自定义事件（webapp）)
...
#### 23.通过圈选定义的元素指标化后（web/app） {#23通过圈选定义的元素指标化后（webapp）}

通过圈选定义的元素，在使用的时候，会指标化为「该元素点击 / 浏览的人数或次数」。

#### 24.自定义事件（web/app） {#24自定义事件（webapp）}
