...
#### 访问用户量（web/app/小程序） {#17}

访问用户的数量。

#### 新访问用户量（web/app/小程序） {#18新访问用户量（webapp）}

（接入 GrowingIO 后）365 天内第一次访问网站/小程序；打开App的访问用户数量。

#### 登录用户量（web/app/小程序） {#19}

登录用户的数量，需要上传登录用户 ID 。

#### 新登录用户量（web/app/小程序） {#20}
...
### 第二部分：访问级指标 {#di-yi-bu-fen-fang-wen-ji-zhi-biao}

> **衡量网站总体访问情况**

#### 访问量（web/app/小程序） {#1fang-wen-liang}

访问的数量。与页面级变量一起使用时，代表着该页面被访问的次数。

#### 访问用户人均访问次数（web/app/小程序） \[ 访问量 / 访问用户 \] {#2}

平均每个用户访问网站（打开 App）的次数。

#### 每次访问页面浏览量（web/app/小程序） \[ 页面浏览量 / 访问量 \] {#3}
...
#### 平均访问时长（web/app/小程序） \[ 总访问时长 / 访问量 \] {#5}
...
#### 访问用户人均进入次数（web/小程序） \[ 进入量 / 访问用户量 \] {#11}
...
#### 平均进入时长（web） \[ 总进入时长 / 进入量 \] {#13}

用户平均每次进入网站/小程序进行访问的平均时长，以分钟作为单位展示。

#### 每次进入页面浏览量（web/小程序） \[ 进入总页面浏览量 / 进入量 \] {#14}
...
#### 页面浏览量（web/app/小程序） {#21}

用户实际浏览过的网页数量。

#### 通过圈选定义的页面浏览量（web/app/小程序） {#22}
...
#### 通过圈选定义的元素指标化后（web/app/小程序） {#23}

通过圈选定义的元素，在使用的时候，会指标化为「该元素点击 / 浏览的人数或次数」。

#### 自定义事件（web/app/小程序） {#24}
...
#### 2.地区名称（web/app/小程序） {#2-di-qu-ming-cheng-webapp-xiao-cheng-xu}
...
#### 5.微信用户所在城市（小程序） {#5-wei-xin-yong-hu-suo-zai-cheng-shi-xiao-cheng-xu}
...
#### 6.微信用户所在省（小程序） {#6-wei-xin-yong-hu-suo-zai-sheng-xiao-cheng-xu}
...
#### 7.微信用户所在国家（小程序） {#7-wei-xin-yong-hu-suo-zai-guo-jia-xiao-cheng-xu}
...
### 第五部分：用户性别 {#di-wu-bu-fen-yong-hu-xing-bie}

#### 1. 微信用户性别 （小程序） {#1-wei-xin-yong-hu-xing-bie-xiao-cheng-xu}
