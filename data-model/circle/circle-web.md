...
* [1.开始圈选](circle-web.md#1开始圈选)
  * [1.1 定义页面](circle-web.md#11-定义页面)
  * [1.2 定义元素](circle-web.md#12-定义元素)
* [2.FAQ](circle-web.md#2faq)
  * [2.1 如何定义“一组同类元素之和”？](circle-web.md#21-如何定义一组同类元素之和？)
  * [2.2 对于已定义过的元素，更改定义规则后，应该如何保存？](circle-web.md#22-对于已定义过的元素，更改定义规则后，应该如何保存？)
  * [2.3 如何进行文本“内容”的模糊匹配？](circle-web.md#22-对于已定义过的元素，更改定义规则后，应该如何保存？)
  * [2.4 3月21日迭代后，新旧版本规则对应关系](circle-web.md#22-对于已定义过的元素，更改定义规则后，应该如何保存？)
* [3.圈选元素的新增文案（web）](circle-web.md#3圈选元素的新增文案（web）)
  * [3.1 元素标签](circle-web.md#31-元素标签)
  * [3.2 圈选到容器的情况](circle-web.md#32-圈选到容器的情况)
* [4.不能圈选的可能原因以及对应方法（web）](circle-web.md#4不能圈选的可能原因以及对应方法（web）)
  * [4.1 不能圈选的原因](circle-web.md#41-不能圈选的原因)
  * [4.2 data-growing-circle属性的使用帮助：](circle-web.md#42-data-growing-circle属性的使用帮助：)
* [常见问题（web）](circle-web.md#常见问题（web）)
...
#### 2.1 如何定义“一组同类元素之和”？ {#21-如何定义一组同类元素之和？}

如果该元素有同类元素，不限定所有的限定条件，即是统计一组同类元素之和的数据。

#### 2.2 对于已定义过的元素，更改定义规则后，应该如何保存？ {#22-对于已定义过的元素，更改定义规则后，应该如何保存？}

更改新的规则后，如果原有数据仍然想继续统计，请选择“另存为”来定义新的规则。

#### 2.3 如何进行文本“内容”的模糊匹配？ {#23-如何进行文本内容的模糊匹配？}
...
### 4.不能圈选的可能原因以及对应方法（web） {#4不能圈选的可能原因以及对应方法（web）}

#### 4.1 不能圈选的原因 {#41-不能圈选的原因}
...
#### 4.2 data-growing-circle属性的使用帮助： {#42-data-growing-circle属性的使用帮助：}
...
#### 1.如果我的页面改版，现在标记的指标是不是需要重新定义？ {#1如果我的页面改版，现在标记的指标是不是需要重新定义？}

改版后，变化的元素需要重新圈选。

#### 2.什么时候选择手动圈选，什么时候选择自动圈选？ {#2什么时候选择手动圈选，什么时候选择自动圈选？}
...
#### 4.为什么圈选的元素只有点击量，没有浏览量？ {#4为什么圈选的元素只有点击量，没有浏览量？}

1. 如果被圈选元素是 a, button, input，img，并且在倒数两层以下，只统计点击量，不统计浏览量。
2. 如果用户使用的是 IE8 及以下的 IE 浏览器版本，无法统计浏览量 ，只统计点击量。

#### 5.为什么圈选的元素只有浏览量，没有点击量？ {#5为什么圈选的元素只有浏览量，没有点击量？}

1. 可能真的是没有点击量。
2. 可能选中的是纯文本等没有带超链接可点击的元素。

#### 6.当页面带查询条件的时候，开关查询条件，数据为什么没有变化？ {#6当页面带查询条件的时候，开关查询条件，数据为什么没有变化？}

当圈选元素所在的页面带查询条件的时候，此时圈选框内展示的数据是不带查询条件的数据。保存带查询条件指标后，指标不会回溯数据，会从保存后开始统计数据。

#### 7.web 圈选时候页面加载不完全？错位？ {#7web-圈选时候页面加载不完全？错位？}

部分浏览器会有兼容性问题，推荐使用 Chrome 浏览器。

#### 8.web 端已经装了 SDK，现在不能进行圈选。 {#8web-端已经装了-sdk，现在不能进行圈选。}
...
#### 9.能否在 iframe 中进行圈选？ {#9能否在-iframe-中进行圈选？}
...
#### 10.在web圈选的时候，为什么有时候会一下圈出一组同类元素，有办法区分开么？\*\* {#10在web圈选的时候，为什么有时候会一下圈出一组同类元素，有办法区分开么？}
...
**仍有疑问？请参考**[**常见问题－圈选部分**](https://docs.growingio.com/faq/circle-metrics.html)
