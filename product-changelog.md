...
### 更新时间 2018 年 12 月 11 日 版本号：V18.23.0 <a id="v-18-11-0"></a>

#### **新特性**

* **H5活动分析**
  * 用户只需要输入一个活动起始页面的 url，就可以看到活动基本数据；
  * 支持设置一个活动目标，既可以选择定义过的指标，也可以选择未定义过的页面；
  * 用户可以直接看到参与活动的用户到完成目标的用户的转化率；
  * 支持对活动进行管理，以及权限控制；
  * 用户可以看到活动的实时数据；
  * 用户可以看到活动的总体数据概览；
  * 用户可以看到 PV / UV / 活动目标 随时间的变化趋势；
  * 用户可以看到参与活动的城市分布、渠道来源、广告来源、设备类型等预定义和自定义变量。
* **分群详情页更新**
  * 老的详情页只支持当前分群和全量用户之间的对比，新版本加入了当前分群与其他分群对比的功能。现在可以支持用户对“转化”和“未转化”人群的对比分析场景了。
* **小程序分享分析**
  * 用户可以了解自己的病毒系数，以及病毒系数的天趋势变化。
* **查询条件直接定义为页面级变量**
  * URL中的查询条件作为页面级变量的取值逻辑调整；
  * 增加了在web圈选中将查询条件创建为页面级变量的入口。

#### **优化**

* **实时**
  * 用户可以看到此时此刻的活跃用户数；
  * 用户可以看到 分城市，平台，渠道的实时数据；
  * 用户可以自定义实时单图，多维监控业务数据；
  * 我们为用户迁移了旧版实时图表的数据，这样用户在新实时中可以看到在旧版定义的实时单图；
  * 小程序用户可以看到基于微信场景的实时监控模板。
* **小程序分享分析**
  * 用户可以看到看到场景描述，了解不同数据回答什么问题；
  * 用户使用过滤条件进行分析时，可以在页面上了解过滤条件是什么。
* **小程序交互圈选&debugger**
  * 仅展示最近5个进入的用户头像，用户可以更方便的定位到自己。

#### 修复

* **微信内嵌页**
  * 修复连续两次新建微信内嵌应用，不刷新的情况下，应用名重复为第一个应用名的问题。
* **小程序分享分析**
  * 修复KOL下载表格仅下载10条而不是1000条的问题。
